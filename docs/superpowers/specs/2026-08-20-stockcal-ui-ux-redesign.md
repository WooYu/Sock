...
1. **可读性优先**：tabular 数字对齐、对比度达 WCAG 2.2、「一眼能读、第一遍就信任」。
2. **智能密度 + 渐进披露**：首页先答最要紧的问题，深度按需展开；减法优于加法。
3. **冷静优于噪音**：颜色用来告知而非惊吓；每屏只留一个主行动。
4. **克制配色**：深色 Midnight Blue 风（近黑海军蓝 + 蓝主色 + 暖金点缀），品牌色与主行动色合一。
5. **扁平高对比**：不用玻璃拟态做基底；游戏化（彩带/徽章）一律不引入。
6. **感知速度**：加载用骨架屏，不用转圈。

## 配色（2026 Midnight Blue 风，替换现有主题色）
...
7. **骨架屏加载**：行情/AI/列表加载态改用骨架屏占位，替代 `CircularProgressIndicator`。
