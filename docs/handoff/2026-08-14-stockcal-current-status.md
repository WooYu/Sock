...
| 设置与后台 | 主题/通知/指标默认参数持久化并全局应用；JSON 数据归档/恢复、本地备份、账户注销；行情/同步任务、用户角色、审计日志、AI 调用日志和密钥状态视图 |
...
- 组合已支持多组合/多资金账户：创建、重命名、删除、切换与跨组合汇总（本次已完成）。
- 数据导入导出当前使用 JSON 文本对话框；面向普通用户的文件选择器、下载与分享仍需补充（需 `file_picker`/`share_plus`，受本机代理/网络限制未完成）。
- 管理后台已支持审计日志与 AI 调用日志的列表视图；规则模板的完整管理操作仍需深化（需明确服务端“规则模板”与知识规则/客户端规则的关系）。
- 主题、通知偏好与指标默认参数已持久化并全局应用。
