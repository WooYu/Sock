...
| 组合与交易 | 买卖、分红、送转、费用；平均成本、已实现/浮动盈亏、现金；CSV 字段映射、预览校验、原子导入和撤销 |
...
| 设置与后台 | 偏好界面、JSON 数据归档/恢复、本地备份、账户注销；行情/同步任务、用户角色、规则模板、审计、AI 日志和密钥状态视图 |
...
- 当前组合账本以单组合为主，多组合和多资金账户的完整切换与汇总仍需扩展。
- 数据导入导出当前使用 JSON 文本对话框；面向普通用户的文件选择器、下载与分享仍需补充。
- 管理后台已有用户、任务、密钥状态及摘要视图，规则模板、完整审计日志和 AI 调用日志的管理操作仍需深化。
- 主题、指标默认参数和通知偏好尚未全部持久化并应用到全局。
...
| 检查 | 结果 |
|---|---|
| `flutter test` | 149 项通过，0 项失败 |
| `flutter analyze` | 通过，0 个问题 |
| `flutter build web --release` | 通过，产物生成于 `build/web` |
| `backend\\gradlew.bat --no-daemon test` | 30 项通过，0 失败、0 错误、0 跳过 |
