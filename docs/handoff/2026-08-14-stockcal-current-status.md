...
| 设置与后台 | 主题/通知偏好持久化并全局应用；JSON 数据归档/恢复、本地备份、账户注销；行情/同步任务、用户角色、审计日志、AI 调用日志和密钥状态视图 |
...
- 管理后台已支持审计日志与 AI 调用日志的列表视图；规则模板的完整管理操作仍需深化。
- 主题与通知偏好已持久化并全局应用；指标默认参数尚未持久化并应用到分析计算。
