...
export default async function AnalysisRoute({ params, searchParams }: { params: Promise<{ tab: string }>; searchParams: Promise<{ symbol?: string }> }) {
  ...
  return <StockWorkspaceProvider initialSymbol={symbol ?? '600519'}><ProductShell activeHref={`/analysis/${tab}`} section="analysis" tone="cyber"><AnalysisPage tab={tab as AnalysisTab} /></ProductShell></StockWorkspaceProvider>
}
