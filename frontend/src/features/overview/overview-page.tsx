  ...
  return (
    <div className="sc-dashboard">
      <section className="sc-section sc-portfolio" aria-labelledby="portfolio-title">
        <SectionHeading eyebrow="组合视角 · 多股票盈亏" title="组合总览" note="演示持仓 · 非真实账户" titleId="portfolio-title" />
        <div className="sc-metrics sc-metrics-six">
          <Metric label="持仓股票" value={holdings.length.toString()} suffix="只" />
          <Metric label="总投入" value="78,570.00" suffix="元" />
          <Metric label="当前市值" value="79,916.00" suffix="元" />
          <Metric label="总浮动盈亏" value="+1,346.00" suffix="元" tone="positive" />
          <Metric label="已实现盈亏" value="+84.00" suffix="元" tone="positive" />
          <Metric label="组合收益率" value="+1.82" suffix="%" tone="positive" hint="浮动 + 已实现" />
        </div>
      ...
      <section className="sc-ticker" aria-labelledby="ticker-title">
        ...
        <div className="sc-cycle"><span>操作周期 <em>系统推荐</em></span><div><button className="active" type="button">短线</button><button type="button">波段</button><button type="button">中长线</button></div></div>
      </section>
      ...
      <section className="sc-section sc-mode" aria-labelledby="mode-title"><div className="sc-mode-intro"><SectionHeading eyebrow="盈利模式识别 · 规则匹配" title="当前更适合哪种盈利方式？" titleId="mode-title" /><p>系统把当天的振幅、MA5、BOLL、量能和趋势状态打成多个参数，再按你的经验规则排序。</p><div className="sc-parameter-strip"><span>今日参数 <b>12</b></span><span>振幅匹配 <b>3.8%</b></span><span>规则命中 <b>8/11</b></span></div></div><div className="sc-mode-list" role="tablist" aria-label="盈利模式">{modes.map((mode, index) => <button className={`sc-mode-option ${mode.tone} ${selectedMode === index ? 'selected' : ''}`} key={mode.name} onClick={() => setSelectedMode(index)} role="tab" aria-selected={selectedMode === index} type="button"><span><b>{mode.name}</b><em>{mode.tag}</em></span><strong><i style={{ width: `${mode.score}%` }} />{mode.score}</strong><small>{mode.description}</small></button>)}</div><div className="sc-mode-detail"><div><span className={`sc-mode-badge ${selected.tone}`}>{selected.tag}</span><h3>{selected.name}</h3></div><button type="button">设为主策略</button><p>{selected.description}</p><div className="sc-levels"><span><small>买入关注</small><b>不追高</b></span><span><small>卖出 / 止盈</small><b>33.88～34.12</b></span><span><small>失效条件</small><b>跌破 5日线且放量</b></span></div><div className="sc-evidence"><span>✓ 距离 MA5 偏离 1.7%</span><span>✓ 上轨附近易反复</span><span>✓ 适合已有持仓</span></div></div></section>

      <section className="sc-section" aria-labelledby="actions-title"><SectionHeading eyebrow="历史数据校准 · 公司行为" title="公司行为调整" note="除权除息后再计算 MA / BOLL" titleId="actions-title" /><div className="sc-actions-grid"><div className="sc-table sc-action-table"><div className="sc-table-row sc-table-head"><span>日期</span><span>行为</span><span>说明</span><span>纳入调整</span></div><div className="sc-table-row"><b>2026-07-21</b><strong>现金分红 0.50 元</strong><span>每股现金分红</span><button className="sc-switch active" aria-label="取消公司行为 2026-07-21" type="button"><i /></button></div><div className="sc-table-row"><b>2026-06-18</b><strong>拆分 2:1</strong><span>10 转 5</span><button className="sc-switch active" aria-label="取消公司行为 2026-06-18" type="button"><i /></button></div></div><div className="sc-action-result"><p className="sc-eyebrow">调整结果</p><div><span>现价参考</span><strong>32.68</strong></div><div className="active"><span>复权参考价</span><strong>16.09</strong></div><p>当前启用 2 项公司行为。调整仅影响历史价格与指标计算，不会修改实际成交记录。</p><button type="button">恢复全部调整</button></div></div></section>

      <section className="sc-ai" aria-labelledby="ai-title"><div className="sc-ai-head"><div><p className="sc-eyebrow">模型编排 · 可解释分析</p><h2 id="ai-title">AI策略分析中心</h2><p>这里展示策略是如何从数值计算、规则匹配和 AI 解释共同得到的。当前仅为演示模型，不代表已调用真实 API。</p></div><span>DEMO · 未接入真实模型</span></div><div className="sc-ai-pipeline"><Pipeline number="01" title="数值计算层" detail="MA · BOLL · ATR · 振幅" value="12 项参数" /><i>→</i><Pipeline number="02" title="规则引擎层" detail="个人经验 · 策略条件" value="8/11 命中" /><i>→</i><Pipeline number="03" title="AI解释层" detail="复杂推理与策略解释" value="86 分" /></div><div className="sc-ai-body"><div className="sc-ai-conclusion"><div className="sc-ai-tabs"><button className="active" type="button">策略结论</button><button type="button">解释依据</button><button type="button">待确认经验</button></div><div className="sc-conclusion-content"><strong>↗</strong><div><span>当前推荐</span><h3>3天5日线买入 → BOLL上轨止盈</h3><p>当前价格位于 MA5 上方，BOLL 上轨持续抬升，近期振幅适合短线区间策略。建议下个交易日等待 32.98～33.24 回踩，不追高；目标位 33.88～34.12。</p><div className="sc-scores"><span>策略匹配度 <b>86</b></span><span>规则可信度 <b>78</b></span><span>风险等级 <b>中等</b></span></div></div></div></div><aside className="sc-model-card"><p className="sc-eyebrow">当前分析模型</p><div><button className="active" type="button">GPT-5</button><button type="button">轻量分类模型</button></div><h3>GPT-5</h3><span>演示模型</span><p>把指标结果、用户规则和历史案例组织成可解释的下个交易日策略。</p><dl><div><dt>用途</dt><dd>复杂推理与策略解释</dd></div><div><dt>输入</dt><dd>指标、规则、历史复盘</dd></div><div><dt>输出</dt><dd>策略排序与解释</dd></div></dl><button type="button">查看模型配置</button></aside></div></section>
      ...
      {workspace.selectedSymbol ? <p className="sc-selected-stock"><span>当前股票：{workspace.selectedSymbol}</span><small>已同步到分析、K 线与交易工作区</small></p> : null}
    </div>
  )
}
...
function Zone({ tone, title, price, range, probability, window, expanded }: { tone: 'rise' | 'fall'; title: string; price: string; range: string; probability: string; window: string; expanded?: boolean }) {
  return <article className={`sc-zone ${tone} ${expanded ? 'expanded' : ''}`}><button type="button" aria-expanded={expanded}><span><b>{title}</b><em>{expanded ? '先看这里' : '观察区'}</em></span><strong>{price}<small>{range}</small></strong><div><span>触达概率 <b>{probability}</b></span><span>观察窗口 <b>{window}</b></span></div><footer><span>共振 · ●●●●</span><span>{expanded ? '收起依据 ⌃' : '展开依据 ⌄'}</span></footer></button>{expanded ? <div className="sc-zone-detail"><p>日线 MA5 上移与周线 BOLL 中轨形成近端共振，经验规则将代表价上调至 {price}。</p><span>触发：放量站稳 33.48，确认向上突破</span><span>失效：回落并收于 32.78 下方</span></div> : null}</article>
}
