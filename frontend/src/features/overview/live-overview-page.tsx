...
export function LiveOverviewPage({ snapshot: providedSnapshot, status: providedStatus, errorMessage: providedError }: LiveOverviewPageProps) {
  ...
  return <div className="sc-live-dashboard sc-decision-workspace">
    <MarketFeedback errorMessage={errorMessage} hasSnapshot onRetry={retry} status={status} />
    <section aria-label="投资决策" className="sc-live-hero sc-decision-primary"><div><p className="sc-eyebrow">实时行情 · {snapshot.source.online ? '在线' : '离线缓存'}</p><h1>{snapshot.quote.security.name}</h1><p className="sc-live-code">{snapshot.quote.security.code}{snapshot.quote.security.exchange ? ` · ${snapshot.quote.security.exchange}` : ''}</p></div><div className="sc-live-price"><strong>{snapshot.quote.price.toFixed(2)}</strong>{change === null ? null : <span className={change >= 0 ? 'sc-positive' : 'sc-negative'}>{change >= 0 ? '+' : ''}{change.toFixed(2)}</span>}</div></section>
    <section aria-label="关键位" className="sc-live-card"><div className="sc-live-card-heading"><h2>关键位</h2><span>{snapshot.source.name} · {new Date(snapshot.source.fetchedAt).toLocaleString('zh-CN')}</span></div><dl className="sc-live-metrics"><div><dt>今开</dt><dd>{formatOptional(snapshot.quote.open)}</dd></div><div><dt>压力</dt><dd>{formatOptional(snapshot.quote.high)}</dd></div><div><dt>支撑</dt><dd>{formatOptional(snapshot.quote.low)}</dd></div><div><dt>成交量</dt><dd>{formatVolume(snapshot.quote.volume)}</dd></div><div><dt>历史 K 线</dt><dd>{snapshot.dailyCandles.length} 根</dd></div></dl></section>
    <section aria-label="下一步" className="sc-live-actions"><Link href={`/chart?symbol=${encodeURIComponent(snapshot.quote.security.code)}`}>打开专业 K 线</Link><Link href={`/analysis/key-levels?symbol=${encodeURIComponent(snapshot.quote.security.code)}`}>查看规则结论</Link><Link href="/rules">管理经验规则</Link></section>
  </div>
}
