...
const tabs: Array<[TradingTab, string]> = [['positions', '持仓'], ['ledger', '交易流水'], ['predictions', '预测记录'], ['statistics', '统计图表']]
...
export function TradingPage({ symbol, initialTab = 'positions' }: { symbol?: string | null; initialTab?: TradingTab }) {
  ...
  const label = tabs.find(([key]) => key === tab)?.[1] ?? '持仓'
  ...
  return <section className="sc-trading-page"><div className="sc-trading-header"><div><p className="text-sm text-[var(--sc-muted)]">{symbol ? `当前股票：${symbol}` : '账户与组合'}</p><h1 className="mt-1 text-2xl font-semibold">{label}</h1></div><RecordSyncButton onSynced={() => { setTrades(loadRecords<TradeRecord>('trades')); setPredictions(loadRecords<PredictionRecord>('predictions')) }} /></div><div className="sc-workspace-tabs" role="tablist" aria-label="交易内容">{tabs.map(([key, name]) => <Link className={tab === key ? 'is-active' : ''} data-active={tab === key} href={`/trading/${key}${symbol ? `?symbol=${symbol}` : ''}`} key={key} onClick={() => setTab(key)} role="tab" aria-selected={tab === key}>{name}</Link>)}</div><div className="sc-trading-content"><TradingContent tab={tab} symbol={symbol} trades={visibleTrades} predictions={visiblePredictions} onTradeSaved={(trade) => setTrades((items) => [...items, trade])} /></div></section>
}
...
function TradingContent({ tab, symbol, trades, predictions, onTradeSaved }: { tab: TradingTab; symbol?: string | null; trades: TradeRecord[]; predictions: PredictionRecord[]; onTradeSaved: (trade: TradeRecord) => void }) {
  if (tab === 'positions') {
    ...
    const positions = Array.from(trades.reduce((items, trade) => {
      const current = items.get(trade.symbol) ?? { symbol: trade.symbol, shares: 0, netCost: 0 }
      const direction = trade.side === 'buy' ? 1 : -1
      current.shares += direction * trade.quantity
      current.netCost += direction * trade.quantity * trade.price
      items.set(trade.symbol, current)
      return items
    }, new Map<string, { symbol: string; shares: number; netCost: number }>()).values()).filter((position) => position.shares !== 0)
    return <div className="sc-trading-view"><div className="sc-trading-stat-grid">{[['持仓股数', `${shares} 股`], ['累计投入', invested ? `¥${invested.toFixed(2)}` : '—'], ['交易笔数', `${trades.length} 笔`]].map(([title, value]) => <article className="sc-trading-stat-card rounded-xl bg-[var(--sc-surface-muted)] p-4" key={title}><p className="text-sm text-[var(--sc-muted)]">{title}</p><p className="mt-2 text-xl font-semibold">{value}</p></article>)}</div><div className="sc-position-table-wrap"><table aria-label="持仓列表" className="sc-position-table"><thead><tr><th>股票</th><th>持仓数量</th><th>平均成本</th><th>操作</th></tr></thead><tbody>{positions.length ? positions.map((position) => <tr key={position.symbol}><td data-label="股票">{position.symbol}</td><td data-label="持仓数量">{position.shares} 股</td><td data-label="平均成本">¥{(position.netCost / position.shares).toFixed(2)}</td><td data-label="操作"><Link href={`/analysis/key-levels?symbol=${position.symbol}`}>查看分析</Link></td></tr>) : <tr><td className="sc-table-empty" colSpan={4}>暂无持仓记录，请先记录一笔交易。</td></tr>}</tbody></table></div><div className="sc-trading-actions flex flex-wrap gap-3"><Link className="min-h-12 rounded-xl bg-[var(--sc-primary)] px-4 py-3 text-sm font-semibold text-white" href={`/analysis/key-levels${symbol ? `?symbol=${symbol}` : ''}`}>查看个股分析</Link><Link className="min-h-12 rounded-xl border border-[var(--sc-border)] px-4 py-3 text-sm font-semibold" href={`/trading/ledger${symbol ? `?symbol=${symbol}` : ''}`}>记录交易</Link><Link className="min-h-12 rounded-xl border border-[var(--sc-border)] px-4 py-3 text-sm font-semibold" href={`/chart${symbol ? `?symbol=${symbol}` : ''}`}>打开 K 线</Link></div></div>
  }
  ...
  return <div className="sc-trading-stat-grid"><article className="sc-trading-stat-card rounded-xl bg-[var(--sc-surface-muted)] p-4"><p className="text-sm text-[var(--sc-muted)]">交易总数</p><p className="mt-2 text-2xl font-semibold">{trades.length}</p></article><article className="sc-trading-stat-card rounded-xl bg-[var(--sc-surface-muted)] p-4"><p className="text-sm text-[var(--sc-muted)]">买入 / 卖出</p><p className="mt-2 text-2xl font-semibold">{buys} / {sells}</p></article><article className="sc-trading-stat-card rounded-xl bg-[var(--sc-surface-muted)] p-4"><p className="text-sm text-[var(--sc-muted)]">预测快照</p><p className="mt-2 text-2xl font-semibold">{predictions.length}</p></article></div>
}
