...
function DailyReview() {
  ...
  return <form className="sc-daily-review-form" onSubmit={save}><div className="sc-daily-review-sections">{dailySections.map((section) => <label className="sc-daily-review-field" key={section}><span>{section}</span><textarea aria-label={section} className="sc-daily-review-input" onChange={(event) => setDraft((current) => ({ ...current, [section]: event.target.value }))} placeholder={section === '市场判断' ? '今天的市场结构、风险与关键位' : section === '执行情况' ? '实际执行、偏差与原因' : '下一次要保留或修正的动作'} value={draft[section]} /></label>)}</div><button className="sc-review-primary-action" type="submit">保存复盘</button>{message ? <p className="text-sm text-emerald-700" role="status">{message}</p> : null}</form>
}
