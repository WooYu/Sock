...
export function ReviewPage({ initialTab = 'daily', tradeId, predictionId }: { initialTab?: ReviewTab; tradeId?: string | null; predictionId?: string | null }) {
  ...
  return <section className="space-y-5 rounded-2xl border border-[var(--sc-border)] bg-[var(--sc-surface)] p-5"><div><RecordSyncButton /><p className="text-sm text-[var(--sc-muted)]">复盘工作台</p><h1 className="mt-1 text-2xl font-semibold">{label}</h1></div><div className="flex min-w-max gap-2 overflow-x-auto" role="tablist" aria-label="复盘内容">{tabs.map(([key, name]) => <Link className="min-h-12 rounded-xl px-4 py-3 text-sm data-[active=true]:bg-[var(--sc-primary)] data-[active=true]:text-white" data-active={tab === key} href={`/review/${key}`} key={key} onClick={() => setTab(key)} role="tab" aria-selected={tab === key}>{name}</Link>)}</div>{tab === 'daily' ? <DailyReview /> : null}{tab === 'trade' ? <TradeReview tradeId={tradeId} predictionId={predictionId} /> : null}{tab === 'history' ? <ReviewHistory /> : null}{tab === 'backtest' ? <Backtest /> : null}</section>
}
