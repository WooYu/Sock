...
describe('RulesPage interactions', () => {
  ...
  test('filters the information-flow list by rule name', async () => {
    ...
    await user.type(screen.getByRole('searchbox', { name: '搜索规则' }), '海龟')
    expect(screen.getByText('大盘暴跌时优先看海龟')).toBeVisible()
    expect(screen.queryByText('5日线上方优先参与')).not.toBeInTheDocument()
  })
